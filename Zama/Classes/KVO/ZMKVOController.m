...
#pragma mark _ZMKVOInfo -

typedef NS_ENUM(uint8_t, _ZMKVOInfoState) {
  _ZMKVOInfoStateInitial = 0,

  // whether the observer registration in Foundation has completed
  _ZMKVOInfoStateObserving,

  // whether `unobserve` was called before observer registration in Foundation has completed
  // this could happen when `NSKeyValueObservingOptionInitial` is one of the NSKeyValueObservingOptions
  _ZMKVOInfoStateNotObserving,
};

NSString *const ZMKVONotificationKeyPathKey = @"ZMKVONotificationKeyPathKey";
 ...
 */
@interface _ZMKVOInfo : NSObject
@end

@implementation _ZMKVOInfo
{
@public
  __weak ZMKVOController *_controller;
  NSString *_keyPath;
  NSKeyValueObservingOptions _options;
  SEL _action;
  void *_context;
  ZMKVONotificationBlock _block;
  _ZMKVOInfoState _state;
}

- (instancetype)initWithController:(ZMKVOController *)controller
                           keyPath:(NSString *)keyPath
                           options:(NSKeyValueObservingOptions)options
                             block:(nullable ZMKVONotificationBlock)block
...
- (instancetype)initWithController:(ZMKVOController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(ZMKVONotificationBlock)block
{
  return [self initWithController:controller keyPath:keyPath options:options block:block action:NULL context:NULL];
}

- (instancetype)initWithController:(ZMKVOController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options action:(SEL)action
{
  return [self initWithController:controller keyPath:keyPath options:options block:NULL action:action context:NULL];
}

- (instancetype)initWithController:(ZMKVOController *)controller keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context
{
  return [self initWithController:controller keyPath:keyPath options:options block:NULL action:NULL context:context];
}

- (instancetype)initWithController:(ZMKVOController *)controller keyPath:(NSString *)keyPath
{
...
- (BOOL)isEqual:(id)object
{
  ...
  return [_keyPath isEqualToString:((_ZMKVOInfo *)object)->_keyPath];
}
...
@implementation _ZMKVOSharedController
{
  NSHashTable<_ZMKVOInfo *> *_infos;
  pthread_mutex_t _mutex;
}

+ (instancetype)sharedController
{
  static _ZMKVOSharedController *_controller = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    _controller = [[_ZMKVOSharedController alloc] init];
  });
  return _controller;
}
...
- (void)observe:(id)object info:(nullable _ZMKVOInfo *)info
{
  ...
  // add observer
  [object zm_addObserver:self forKeyPath:info->_keyPath options:info->_options context:(void *)info];

  if (info->_state == _ZMKVOInfoStateInitial) {
    info->_state = _ZMKVOInfoStateObserving;
  } else if (info->_state == _ZMKVOInfoStateNotObserving) {
    // this could happen when `NSKeyValueObservingOptionInitial` is one of the NSKeyValueObservingOptions,
    // and the observer is unregistered within the callback block.
    // at this time the object has been registered as an observer (in Foundation KVO),
    // so we can safely unobserve it.
    [object zm_removeObserver:self forKeyPath:info->_keyPath];
  }
}
...
- (void)unobserve:(id)object info:(nullable _ZMKVOInfo *)info
{
  ...
  // remove observer
  if (info->_state == _ZMKVOInfoStateObserving) {
    [object zm_removeObserver:self forKeyPath:info->_keyPath];
  }
  info->_state = _ZMKVOInfoStateNotObserving;
}

- (void)unobserve:(id)object infos:(nullable NSSet<_ZMKVOInfo *> *)infos
{
  if (0 == infos.count) {
    return;
  }
                       ...
                       context:(nullable void *)context
{
  ...
  if (nil != info) {
    ...
    ZMKVOController *controller = info->_controller;
    if (nil != controller) {
      ...
      if (nil != observer) {
        ...
        if (info->_block) {
          ...
          if (keyPath) {
            NSMutableDictionary<NSString *, id> *mChange = [NSMutableDictionary dictionaryWithObject:keyPath forKey:ZMKVONotificationKeyPathKey];
...
#pragma mark ZMKVOController -

@implementation ZMKVOController
{
  NSMapTable<id, NSMutableSet<_ZMKVOInfo *> *> *_objectInfosMap;
...
- (NSString *)debugDescription
{
  ...
  for (id object in _objectInfosMap) {
    ...
    [infos enumerateObjectsUsingBlock:^(_ZMKVOInfo *info, BOOL *stop) {
      [infoDescriptions addObject:info.debugDescription];
    }];
...
- (void)_observe:(id)object info:(_ZMKVOInfo *)info
{
  // lock
  pthread_mutex_lock(&_lock);
  ...
  _ZMKVOInfo *existingInfo = [infos member:info];
  if (nil != existingInfo) {
  ...
  [[_ZMKVOSharedController sharedController] observe:object info:info];
}

- (void)_unobserve:(id)object info:(_ZMKVOInfo *)info
{
  // lock
  pthread_mutex_lock(&_lock);
  ...
  _ZMKVOInfo *registeredInfo = [infos member:info];
  ...
  // unobserve
  [[_ZMKVOSharedController sharedController] unobserve:object info:registeredInfo];
}
...
- (void)_unobserve:(id)object
{
  ...
  // unobserve
  [[_ZMKVOSharedController sharedController] unobserve:object infos:infos];
}
...
- (void)_unobserveAll
{
  ...
  _ZMKVOSharedController *shareController = [_ZMKVOSharedController sharedController];
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(ZMKVONotificationBlock)block
{
  ...
  // create info
  _ZMKVOInfo *info = [[_ZMKVOInfo alloc] initWithController:self keyPath:keyPath options:options block:block];
...
- (void)observe:(nullable id)object keyPaths:(NSArray<NSString *> *)keyPaths options:(NSKeyValueObservingOptions)options block:(ZMKVONotificationBlock)block
{
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options action:(SEL)action
{
  ...
  // create info
  _ZMKVOInfo *info = [[_ZMKVOInfo alloc] initWithController:self keyPath:keyPath options:options action:action];
...
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(nullable void *)context
{
  ...
  // create info
  _ZMKVOInfo *info = [[_ZMKVOInfo alloc] initWithController:self keyPath:keyPath options:options context:context];
...
- (void)unobserve:(nullable id)object keyPath:(NSString *)keyPath
{
  ...
  _ZMKVOInfo *info = [[_ZMKVOInfo alloc] initWithController:self keyPath:keyPath];
