...
/**
 ...
 ZMKVOKeyPath(string.length) => @"length"

 Or even the complex case:

 ZMKVOKeyPath(string.lowercaseString.length) => @"lowercaseString.length".
 */
#define ZMKVOKeyPath(KEYPATH) \
@(((void)(NO && ((void)KEYPATH, NO)), \
({ const char *zmkvokeypath = strchr(#KEYPATH, '.'); NSCAssert(zmkvokeypath, @"Provided key path is invalid."); zmkvokeypath + 1; })))
...
/**
 ...
 ZMKVOClassKeyPath(NSString, length) => @"length"
 ZMKVOClassKeyPath(NSString, lowercaseString.length) => @"lowercaseString.length"
 */
#define ZMKVOClassKeyPath(CLASS, KEYPATH) \
@(((void)(NO && ((void)((CLASS *)(nil)).KEYPATH, NO)), #KEYPATH))

NS_ASSUME_NONNULL_BEGIN

/**
 Key provided in the @c change dictionary of @c ZMKVONotificationBlock that's value represents the key-path being observed
 */
extern NSString *const ZMKVONotificationKeyPathKey;
...
/**
 ...
 @param change The change dictionary which also includes @c ZMKVONotificationKeyPathKey
 */
typedef void (^ZMKVONotificationBlock)(id _Nullable observer, id object, NSDictionary<NSKeyValueChangeKey, id> *change);

/**
 @abstract ZMKVOController makes Key-Value Observing simpler and safer.
 @discussion ZMKVOController adds support for handling key-value changes with blocks and custom actions, as well as the NSKeyValueObserving callback. Notification will never message a deallocated observer. Observer removal never throws exceptions, and observers are removed implicitly on controller deallocation. ZMKVOController is also thread safe. When used in a concurrent environment, it protects observers from possible resurrection and avoids ensuing crash. By default, the controller maintains a strong reference to objects observed.
 */
