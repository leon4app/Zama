...
#define __XXHookClassBegin(theHookClass,                                                                        \
    ...
    __unused Class __xxHookClass = zamazenta_hook_getClassFromObject(theHookClass);                                \
    ...
    ^returnType(theSelfTypeAndVar FOREACH_ARGS(CREATE_ARGS,1,##__VA_ARGS__ )) {                                 \
            ...
            (if (!zamazenta_hook_check_block(object_getClass(self),__xxHookClass,&associatedKey))                  \
             {                                                                                                  \
              XXHookTypeIsVoidType(returnType)                                                                  \
...
#define __xxHookClassEnd                                                                                        \
    ...
    __xx_hook_orgin_function = zamazenta_hook_imp_function(__xxHookClass,                                          \
                                                         __xxHookSel,                                           \
...
#define XXStaticHookPrivateClass(theHookClassType,publicType,GroupName,returnType,theSEL,... )                  \
        __XXStaticHookClass(metamacro_concat(__zamazenta_hook_auto_load_function_ , __COUNTER__),                  \
